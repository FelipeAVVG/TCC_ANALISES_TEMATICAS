...
def exibir(df):
    st.subheader("Análise de Tendências — Artigos Científicos")
    st.info("Identifica temas em ascensão e declínio com base na produção histórica de artigos.")
    ...
    if st.button("Executar Análise de Tendências", type="primary"):
        with st.spinner("Processando dados e treinando modelos..."):
            ...
            if not df_tend.empty:
                ...
                st.plotly_chart(fig, config={'responsive': True}, key="ate_1")

                df_display = df_tend[['tema_simples', 'ultimo_valor', 'previsao_media', 'percentual_mudanca', 'classificacao']].copy()
                df_display.columns = ['Tema', 'Artigos Atuais', 'Previsão Média', 'Mudança %', 'Tendência']
                df_display = df_display.sort_values('Mudança %', ascending=False)
                df_display['Artigos Atuais'] = df_display['Artigos Atuais'].round(0).astype(int)
            ...
            if not df_emerg.empty:
                ...
                with col_left:
                    ...
                    st.plotly_chart(fig2, config={'responsive': True}, key="ate_2")
                with col_right:
                ...
                if tema_viz:
                    ...
                    if len(df_hist) >= 2:
                        ...
                        fig3 = px.line(pd.concat([df_hist, df_prev]), x='ano', y='count', color='tipo',
                                       markers=True, labels={'count': 'Quantidade de Artigos', 'ano': 'Ano'})
                        fig3.update_layout(height=400)
                        st.plotly_chart(fig3, config={'responsive': True}, key="ate_3")
                    else:
        ...
        with col3:
            st.write("**Previsão**")
            st.write("Projeção de artigos para os próximos anos")