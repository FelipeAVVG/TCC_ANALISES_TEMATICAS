...
def exibir(df):
    st.subheader("Análise por Instituição — Artigos Científicos")
    ...
    fig = px.bar(top_inst.head(15), x='Artigos', y='Instituição', orientation='h',
                 labels={'Artigos': 'Quantidade', 'Instituição': ''},
    ...
    st.plotly_chart(fig, config={'responsive': True}, key="ain_1")

    st.markdown("---")

    # Produção anual por instituição
    ...
    fig2 = px.line(df_tempo, x='ano', y='count', color='instituicao', markers=True,
                   labels={'count': 'Artigos', 'ano': 'Ano', 'instituicao': 'Instituição'})
    fig2.update_layout(height=400, hovermode='x unified')
    st.plotly_chart(fig2, config={'responsive': True}, key="ain_2")
    ...
    if inst_sel:
        ...
        with col_a:
            st.metric("Total de Artigos", len(df_inst))
        with col_b:
