...
def main():
    ...
    try:
        with sqlite3.connect(RAW_DB_NAME) as conn:
            ...
            # extrai artigos (caso a tabela tenha sido criada pelo scraper atualizado)
            try:
                df_artigos_raw = pd.read_sql_query("SELECT * FROM artigos", conn)
            except Exception:
                df_artigos_raw = pd.DataFrame()
            # extrai projetos (caso a tabela tenha sido criada pelo scraper atualizado)
            try:
    ...
    # Se havia artigos brutos, prepare também o fato de artigos
    fato_artigo = None
    if not df_artigos_raw.empty:
        print("   - Transformando dados de artigos científicos...")
        # apenas normalização mínima, não aplicamos validação de instituição
        ...
        fato_artigo = df_art[['artigo_id', 'slug_professor', 'nome_professor', 'sigla', 'ano', 'titulo', 'journal', 'doi', 'palavras_chaves']]

    # Se havia projetos brutos, prepare também o fato de projetos
    fato_projeto = None
    if not df_projetos_raw.empty:
